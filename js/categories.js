      ...
      return `
        <div class="book-card" data-id="${item.itemID}" tabindex="0">
          ...
          <div class="book-info">
            ...
            <p class="book-author">${item.contributor ? item.contributor.join(", ") : "Unknown"}</p>
    ...
    const showItemDetails = (id) => {
      ...
      const contentContainer = document.querySelector(".content-container") || document.querySelector(".cat-results");
        ...
        <div class="details-card">
          ...
          <div class="details-info">
            <div class="title-rating">
              <h2>${item.title}</h2>
              <div class="price">Price: $${Number(item.resell_price).toFixed(2)}</div>
              <div class="contributor">By: ${item.contributor.join(", ")}</div>
              <div class="stars">${"★".repeat(Math.round(item.rating || 0))} ${item.rating.toFixed(1)}</div>
    ...
    const backToHomepage = () => {
      ...
      const container = document.querySelector(".content-container") || document.querySelector(".cat-results");
    ...
    const renderFilteredBooks = (items, tag) => {
      ...
      const container = isCategoryPage ? document.querySelector(".cat-results") : document.querySelector(".content-container");
      if (!container) return;
